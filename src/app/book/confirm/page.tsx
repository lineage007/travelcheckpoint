...
function ConfirmInner() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'var(--font-body)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ maxWidth: '480px', width: '100%' }}>
        {state === 'working' && (
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '16px', padding: '40px', textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '16px', fontWeight: 700, color: C.text, marginBottom: '6px' }}>Finalizing your booking…</div>
        ...
        {state === 'done' && result && (
          <div style={{ background: C.card, border: '1px solid rgba(34,197,94,0.25)', borderRadius: '16px', padding: '28px' }}>
            <div style={{ textAlign: 'center', marginBottom: '18px' }}>
              ...
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: 700, color: C.text }}>{result.status === 'CONFIRMED' ? 'Booking confirmed' : `Booking ${result.status?.toLowerCase() || 'received'}`}</div>
            ...
            <div style={{ display: 'grid', gap: '10px', fontSize: '13px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: C.sub }}>Total</span><span style={{ color: C.text, fontFamily: 'var(--font-mono)', fontWeight: 700 }}>{result.currency} {result.price?.toLocaleString()}</span></div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: C.sub }}>Booking ID</span><span style={{ color: C.text, fontFamily: 'var(--font-mono)' }}>{result.bookingId}</span></div>
              {result.hotelConfirmationCode && <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: C.sub }}>Hotel confirmation</span><span style={{ color: C.text, fontFamily: 'var(--font-mono)' }}>{result.hotelConfirmationCode}</span></div>}
        ...
        {state === 'failed' && (
          <div style={{ background: C.card, border: '1px solid rgba(239,68,68,0.25)', borderRadius: '16px', padding: '28px', textAlign: 'center' }}>
            ...
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '18px', fontWeight: 700, color: C.text, marginBottom: '8px' }}>Booking not completed</div>
